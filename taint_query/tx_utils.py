...
from tx_storm import StormOptimizer
...
class SignOpt():
    def __init__(self, x0, target, play_name, output_path, use_storm = False):
        ...
        if use_storm is not None:
            self.storm_optim = StormOptimizer()
    ...
    def sign_opt_search(self, decision):
        ...
        if self.sign_opt_stage == 1:
            ...
            if self.perb_count < 10:
                ...
                #update theta
                if self.use_storm:
                    new_theta = self.storm_optim.step(self.alpha_temp, self.xg, self.sign_gradient)
                else:
